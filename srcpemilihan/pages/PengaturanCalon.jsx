...
export default function PengaturanCalon() {
    ...
    const loadData = async () => {
      try {
        ...
        for (const candidate of candidates) {
          const hasVote = await checkCandidateHasVotes(candidate.id);
          statusMap[candidate.id] = hasVote;
        }
  ...
  const handleEdit = (candidate) => {
    setFormData({
      ...
      alamatKetua: candidate.alamatKetua || "",
      namaWakil: candidate.namaWakil || "",
      alamatWakil: candidate.alamatWakil || "",
      ...
      // AMBIL DATA TAMBAHAN
      dataDiri: candidate.dataDiri || "",
      programKerja: candidate.programKerja || "",
    });

    setEditingId(candidate.id);
    setImagePreview(candidate.foto || null);
    setShowModal(true);
  };

  const handleDelete = async (candidate) => {
    try {
      const votes = await hasVotes(candidate.id);
      if (votes) {
        Swal.fire({
          icon: "error",
      ...
      const result = await Swal.fire({
        icon: "warning",
        title: "Hapus Calon?",
        text: `Apakah Anda yakin ingin menghapus ${candidate.namaKetua}?`,
      ...
      if (result.isConfirmed) {
        ...
        setCalon(calon.filter((c) => c.id !== candidate.id));

        Swal.fire({
          icon: "success",
          title: "Berhasil!",
          text: "Calon berhasil dihapus",
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    name="dataDiri"
                    value={formData.dataDiri}
                    onChange={handleChange}
                    rows={4}
                    placeholder="Tuliskan data diri calon secara singkat..."
                    className="w-full px-4 py-3 rounded-lg bg-[var(--color-background)] border border-[var(--color-border)] resize-none"
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    name="programKerja"
                    value={formData.programKerja}
                    onChange={handleChange}
                    rows={5}
                    placeholder="Tuliskan program kerja calon..."
                    className="w-full px-4 py-3 rounded-lg bg-[var(--color-background)] border border-[var(--color-border)] resize-none"
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-3">
                    {imagePreview ? (
                      ...
                      <label className="flex items-center justify-center w-full h-32 px-4 transition bg-[var(--color-background)] border-2 border-[var(--color-border)] border-dashed rounded-lg cursor-pointer hover:border-[var(--color-primary)]">
                ...
                <div className="flex gap-2 pt-4">
                  <button
                    type="button"
                    onClick={resetForm}
                    className="flex-1 px-4 py-3 rounded-lg bg-gray-300/20 text-gray-600 font-medium hover:bg-gray-300/30 transition-all"
                  >
                    Batal
                  </button>
                  <button
                    type="submit"
                    className="flex-1 px-4 py-3 rounded-lg bg-[var(--color-primary)] text-white font-medium hover:opacity-90 transition-all flex items-center justify-center gap-2"
                  >
