...
export default function DokumenCalonPemilih() {
  ...
  const exportPDF = () => {
    try {
      ...
      doc.text(getJudulLengkap(), pageWidth / 2, currentY + 7, {
        align: "center",
      });
  ...
  const exportExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Daftar Calon Pemilih");
      XLSX.writeFile(
        wb,
        `Daftar-Calon-Pemilih${
          getFilterLabel() ? "-" + getFilterLabel() : ""
  ...
  const getJudulLengkap = () => {
    const label = getFilterLabel();
    return `DAFTAR CALON PEMILIH (PCP)${label ? " - " + label : ""}`;
  };
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-gray-800 text-white">
                ...
                <th className="border p-3 w-24">Nomor Rumah</th>
